  ...
  <?php require_once './view/modals/resa_warning.php'; ?>
  ...
  <form name="form" class="form-accueil row fs-6 container justify-content-center" method="POST">
      <div style='background-color: #fff;' class="col-2 px-0 ">
          <div class="input-group justify-content-center ">
              <label for="id_agence" class="form-label py-0 mt-2">Adresse de départ</label>
  ...
  <!-------------------------------------------------------------------------------->
  <!-------------------NBR RESULTATS + TRI PAR ORDRE DE PRIX------------------------->
  <!-------------------------------------------------------------------------------->
  ...
  <div class="nav_prix">
      ...
      <form id="form" class="d-flex justify-content-around fs-3 f" method="POST" onchange="submit();">
          <p> <?= count($allVehicules); ?> résultat<?php if (count($allVehicules) !== 1) {; ?>s<?php }; ?></p>
          <?php if (count($allVehicules) == 1) {
            }; ?>
          <div class="col-1">
              ...
              <div style="font-family: 'Montserrat'" class="col-6 mt-4 fw-bold">
                  <div class="card-body ms-5 my-4">
                      ...
                      <?php
                        ...
                        ?> <a type="button " class="btn btn-success px-3 py-1" data-bs-toggle="modal" data-bs-target="#checkout">Réserver et payer</a> <?php
                                                                                                                                                    } else {
                                                                                                                                                        if (isset($_POST['id_agence']) && $_POST['id_agence'] && isset($_POST['date_debut']) && $_POST['date_debut'] && isset($_POST['date_fin']) && $_POST['date_fin']) {
                                                                                                                                                        ?>
                              <a type="button " class="btn btn-success px-3 py-1" href="http://127.0.0.1/Location_Veville/?route=reservation&id_vehicule=<?= $value['id_vehicule']; ?>&id_membre=<?= $_SESSION['membre']['id_membre']; ?>&debut=<?= date('d-m-Y', strtotime($_POST['date_debut'])); ?>&fin=<?= date('d-m-Y', strtotime($_POST['date_fin'])); ?>">Réserver et payer</a> <?php } else { ?> <a type="button " class="btn btn-success px-3 py-1" data-bs-toggle="modal" data-bs-target="#resa_warning">Réserver et payer</a>
                  </div>
          <?php }
                                                                                                                                                                                                                                                                                                                                                                                }
            ?>
